...
const interactFunctions = {
    fridgeInteract: (fridge)=>{
        ...
        fridge.children.find(x => x.name == 'light').intensity = fridge.opened ? 1 : 0;
    ...
    stoveInteract: (stove)=>{
        ...
        const stoveBurner = stove.children.find(x => x.name == 'stove_burner');
        stoveBurner.material = stove.on ? stove.heatedBurnerMaterial : stove.originalBurnerMaterial;
    },
    lampInteract: (lamp)=>{
        if(!lamp.on) lamp.on = true;
        else lamp.on = false;

        lamp.children.find(x => x.name == 'light').intensity = lamp.on ? 1 : 0;
    }
}
...
function loadInteractions(){
    ...
    scene.children.map(x => {
        ...
        if(x.name == 'fridge'){
            ...
            const fridgeLightHelper = new THREE.PointLightHelper(fridgeLight,0.2);
        ...
        if(x.name == 'stove'){
            ...
            x.heatedBurnerMaterial = new THREE.MeshBasicMaterial({color:0xaa1111});
        }
        else if(x.name == 'lamp'){
...
function startLoad(){
    ...
    const outlinePass = new OutlinePass(new THREE.Vector2(window.innerWidth, window.innerHeight), scene, cam);
    ...
    window.addEventListener("mousedown", (e) => {
        if(document.pointerLockElement){
            ...
            if(items.length != 0){
                ...
                nameText.innerText = jsonSelected ? jsonSelected.name : selected.name;
                descriptionText.innerText = jsonSelected ? jsonSelected.description : '';
